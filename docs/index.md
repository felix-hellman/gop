...
project:
  package: #This is the package we want to upload
    name: awesome
    author: felix-hellman #This needs to match your github username
    version: 0.0.1
...
#### Tips for running in a pipeline
Instead of doing the login flow manually a token can be passed to the login command
This will bypass the login flow
```bash
gop login --token=ey...
```